...
class Solution {
public:
    void helper(int index, int target, vector<vector<int>>&ans, vector<int>&ds, vector<int>&arr){
        if(target == 0)
        {
            ans.push_back(ds);
            return ;
        ...
        for(int i=index;i<arr.size();i++){
            ...
            ds.push_back(arr[i]);
            helper(i+1, target - arr[i], ans, ds, arr);
            ds.pop_back();
        }
    }
    vector<vector<int>> combinationSum2(vector<int>& candidates, int target) {
        ...
        vector<vector<int>> ans;
        vector<int> ds;
        helper(0, target, ans, ds, candidates);
        return ans;
    }
};
